  ...
  listings.forEach((listing) => {
    ...
    try {
      ...
      if (typeof listing.amenities === "string") {
        ...
        amenities = parsedAmenities.slice(0, 6).join(", ");
      }

      // If already array
      else if (Array.isArray(listing.amenities)) {
        amenities = listing.amenities.slice(0, 6).join(", ");
      }
    } catch (error) {
    ...
    // ================================
    const hostName =
      listing.host_name || listing.host?.host_name || "Unknown Host";
    ...
    // PRICE
    let price = listing.price?.$numberDecimal || listing.price || "N/A";
